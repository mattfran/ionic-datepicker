angular.module('ionic-datepicker.provider', [])
  ...
  .provider('ionicDatePicker', function () {
    ...
    this.$get = ['$rootScope', '$ionicPopup', '$ionicPopover', '$ionicModal', 'IonicDatepickerService', function ($rootScope, $ionicPopup, $ionicPopover, $ionicModal, IonicDatepickerService) {
      ...
      function openPopover(targetElementOrEvent) {
        return $scope.popover.show(angular.element(document.querySelector(targetElementOrEvent)));
      }
      ...
      provider.openDatePicker = function (ipObj, targetElementOrEvent) {
        var buttons = [];
        ...
        if ($scope.mainObj.templateType.toLowerCase() == 'popup') {
          ...
          openPopover(targetElementOrEvent);
        } else {
          openModal();
        }
      };
